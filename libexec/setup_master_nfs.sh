...
if (( ${ENABLE_NFS_GCS_FILE_CACHE} )) ; then
  ...
  chown hadoop:hadoop "${NFS_EXPORT_POINT}" "${META_CACHE_DIRECTORY}"
  ...
  if ! grep -e "BDUTIL_HADOOP_EXPORT" /etc/exports ; then
    if ! id -u hadoop; then
      echo "ERROR: no 'hadoop' user for anonuid in setup_master_nfs.sh"
      exit 1
    fi
    readonly HADOOP_UID="$(id -u hadoop)"
    readonly HADOOP_GID="$(id -g hadoop)"
    OPTIONS_STRING="(rw,all_squash,anonuid=${HADOOP_UID}"
    OPTIONS_STRING="${OPTIONS_STRING},anongid=${HADOOP_GID})"
